...
getwd()
ourPath.a = "../../data/exp3_realism/a/"
ourPath.b = "../../data/exp3_realism/b/"
...
summary(a.m.extra.0)
hypothesis(a.m.extra.0, hypothesis = "vision1 - audio2 > 0")
hypothesis(a.m.extra.0, hypothesis = "vision1 - vibration2 > 0")
```
...
```{r}
a.m.2.predictions = ggpredict(a.m.2, terms = c("offset", "signals"))
a.m.3.predictions = ggpredict(a.m.3, terms = c("offset", "realism"))

signal.line.plot = ggplot(a.m.2.predictions, 
  ...
  scale_y_continuous(name = "Causal Response", breaks = seq(1,9,1)) +
  scale_x_continuous(name = "Offset (ms)") +
  theme(text = element_text(size = 11),
        axis.text.x = element_text(size = 8),
        axis.text.y = element_text(size = 8), aspect.ratio = 0.75)

realism.line.plot = ggplot(a.m.3.predictions, 
       aes(x = x, y = predicted, color = group)) +
  # predictions
  geom_ribbon(mapping = aes(ymin = conf.low,
                            ymax = conf.high,
  ...
  # base data with mean and confidence interval
  stat_summary(data = data.3.a.df,
               mapping = aes(x = offset, y = response, color = realism),
               fun.data = "mean_cl_boot",
               size = 0.35, alpha = 0.75) + 
  # colors
  scale_colour_manual(values = realismPalette, 
                      # labels = c("Real", "Unreal")) +
                      labels = c("Realistic", "Unrealistic")) +
  scale_fill_manual(values = realismPalette) +
  # scales
  coord_cartesian(xlim = c(0, 400), ylim = c(1,9)) +
...
location.line.plot = ggplot(data.3.b.df, 
                            mapping = aes(x = offset, y = response,
                                          shape = factor(location),
                                          color = factor(realism))) +
  stat_summary(fun.data = "mean_cl_boot", #linewidth = 0.25,
               size = 0.5, alpha = 0.5) + 
  # colors
  scale_colour_manual(values = realismPalette, 
                      labels = c("Realistic", "Unrealistic"),
